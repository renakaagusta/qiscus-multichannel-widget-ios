...
extension NetworkManager {
    ...
    func loadComments(roomId: String, lastCommentId: Int, after: Bool? = nil, limit: Int? = 20, completion: @escaping ([CommentModel]?, QError?) -> Void) {
                ...
                case .failure(let errorMessage):
                    do {
                        if let data = data{
                            let jsondata = try JSONSerialization.jsonObject(with: data, options: .mutableContainers)
                            QiscusLogger.errorPrint("json: \(jsondata)")
                            self.event_report(moduleName: "API", event: "update_comment_status", message: "commentReceivedId=\(commentReceivedId), commentReadId=\(commentReadId), error: \(jsondata)", onSuccess: { (success) in
                        ...
                        self.event_report(moduleName: "API", event: "update_comment_status", message: "commentReceivedId=\(commentReceivedId), commentReadId=\(commentReadId), Code =\(response.statusCode), error \(error.localizedDescription)", onSuccess: { (success) in
